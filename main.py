...
def run_marketing_automation(subreddit_name):
    ...
    print(f"[INFO] Starting Marketing Automation Pipeline for r/{subreddit_name}...\n")
    ...
    if not raw_html:
        print("[INFO] No data scraped. Exiting pipeline.")
        return
    
    raw_posts = scraper.parse_data(raw_html)
    print(f"[INFO]  Scraped {len(raw_posts)} posts from r/{subreddit_name}.\n")
    

    # Step 2: Process the scraped data - Transform
    print("[INFO]  Processing and cleaning data...\n")
